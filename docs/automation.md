...
The unresolved **reason** decides what happens next, and the summary spells each one out: `multiple_candidates` is resolvable by hand from `track_position` and the reported candidates; `zero_candidates` means no MySQL row agrees at every non-U+FFFD position (the row changed since, or the character count was not preserved); `corrupt_candidates` means tubafrenzy carries U+FFFD at that position too and the bytes are gone on **both** sides — **unrecoverable**, and not worth re-running for.
