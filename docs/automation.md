...
**`va_release` re-derivation (#344):** before the recall-index build below, `sync-library.sh` runs `scripts/derive_va_release.py` (this repo's own script; the workflow installs this repo's deps before the sync so psycopg is importable) to re-derive the `va_release` VA-compilation lookup table in the discogs-cache — the table the recall-index build's comp-title matcher reads. This daily re-derivation is deliberate, not just a backfill: LML inserts API-fetched VA releases into the cache at runtime, and the monthly rebuild's re-derivation (pipeline step 9) only runs when a rebuild actually happens. The script's floor guard (`VA_RELEASE_FLOOR`, default `1`; `0` opts out) refuses to replace a pre-existing table with a derivation below the floor — the transaction rolls back, keeping yesterday's table — but is skipped when no table exists yet, so the first run always creates one. Soft-fail like its neighbors: a derivation failure logs a `WARNING` and never blocks the library.db upload or the recall-index build (which then reads the previous derivation).
