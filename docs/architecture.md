...
Neither new #370 flag appears above, and both omissions are deliberate. `--residue-ledger` already defaults to the vendored ledger, so naming it here would only substitute a cwd-relative path for a module-relative one that cannot break. `--fail-on-drift` **would make this block exit 4 every single time it is run today** — see the two bullets below.
...
- **`--fail-on-drift`** turns a non-clean verdict into exit 4, which is what a scheduled soak workflow would gate on — see "Unwired secrets (catalog parity)" in [`docs/automation.md`](automation.md) for why no such workflow exists yet. **Do not add it to the invocation above until plan step 6 lands.** The shipped ledger's `baselines.cta_missing` / `baselines.cta_extra` are `null`, and the CTA check requires both to be populated, so `clean` is `false` today *even for two byte-identical `library.db` files* — adding the flag now yields exit 4 on every run, and a workflow copied from this block would be red from its first tick. That is deliberate ("until step 6 measures the CTA baseline, `clean` cannot be true"), but it makes the flag useless until the measurement exists.
