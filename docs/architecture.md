...
**Mechanism**: `scripts/run_pipeline.py::write_keep_release_ids` reads the override table once per pipeline run (read-only, existence-gated via `to_regclass('lml_cache.library_release_override')` -- degrades to an empty allowlist file when the table is absent, which is every DB today) and writes a newline-separated `release_id` allowlist file. That file's path is threaded via `--keep-release-ids <FILE>` into every `dedup_releases.py` invocation and every `verify_cache.py` invocation (`--prune`, `--copy-to`, and the post-import `--prune` path). Omitting the flag (or passing an empty/missing file) is a no-op -- byte-identical to pre-#327 behavior; this is why a standalone/manual run of either script outside the orchestrator applies no exemption.
