...
- `scripts/catalog_parity_diff.py` -- Diff-two-files core of the [#346](https://github.com/WXYC/discogs-etl/issues/346) catalog-parity harness (a distinct check from `wxyc_library_parity_check.py` above): given `--mysql-db` and `--backend-db` paths to two already-built `library.db` SQLite files, diffs them id-keyed across the daily-sync `library` columns (all except `label`, which is always NULL in prod and excluded as a no-op), the optional `compilation_track_artist` table, and row-set membership. Normalizes NULL/`''`/the literal `'NULL'` string/whitespace as equal, and since [#370](https://github.com/WXYC/discogs-etl/issues/370) also runs a **per-column expectation model** (`COLUMN_MODELS`) that replays Backend's own `library-etl` transforms — ported to `lib/backend_catalog_norm.py` from `jobs/library-etl/job.ts` — against the raw MySQL value. Each matched field lands in one of three tiers: *agree* (byte-identical), *normalized* (Backend equals the value its own ETL would derive — a deliberate transform, counted but not drift), or *mismatch* (Backend disagrees with its own ETL's spec — a genuine defect). **`field_mismatches` counts only the mismatch tier**, so its meaning changed with #370: a column differing solely by a deliberate normalization (case folding, VA collapsing, a coerced NULL, a tab/newline substitution) no longer contributes, and the value drops from ~30,700 to ~700 against a healthy production pair. The normalized-tier counts (column → class → count) are emitted on an INFO log line; they join `--json` with the residue ledger. `--json` emits `matched`/`missing_in_backend(_ids)`/`extra_in_backend(_ids)`/`field_mismatches`/`cta_missing`/`cta_extra` so an operator can build the expected-residue ledger. Read-only with respect to both inputs (opens them `mode=ro`); exits 2 on bad args, 3 on a missing/unreadable file or table or a producer failure, 0 otherwise (a nonzero diff is still exit 0 -- the operator reads the counts). It also carries the **producer** half ([#351](https://github.com/WXYC/discogs-etl/issues/351)): `--mysql-source mysql://user@host:port/dbname` (password via `$LIBRARY_DB_PASSWORD`) and `--backend-source https://api.wxyc.org` build the corresponding side first (into the matching `--mysql-db` / `--backend-db` path), so one invocation can build both and diff them. See "Catalog parity producers" below. The *operational* cutover -- running the 7 clean parity days, flipping `sync-library.sh`, taking `/wxycdb` dark -- stays out of scope in [#346](https://github.com/WXYC/discogs-etl/issues/346), deliberately human-gated.
