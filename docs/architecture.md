...
python scripts/catalog_parity_diff.py \
    ...
    --residue-ledger vendor/parity-residue/ledger.json \
    --fail-on-drift \
    --json
```
...
- **`--residue-ledger`** is shown explicitly above but is optional in practice — it already defaults to `vendor/parity-residue/ledger.json`, resolved relative to `catalog_parity_diff.py`'s own location rather than the invoking shell's `cwd`, so the `mktemp -d` pattern above works unchanged no matter where `PARITY_DIR` lands. Pass `--residue-ledger none` to run without one (row-level expected/unexplained classification and `clean` are then unavailable, reporting `null`).
- **`--fail-on-drift`** turns a non-clean verdict into exit 4, which is what a scheduled soak workflow would gate on — see "Unwired secrets (catalog parity)" in [`docs/automation.md`](automation.md) for why no such workflow exists yet.
