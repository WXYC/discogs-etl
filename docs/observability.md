...
Scripts that initialize the logger (subprocesses each get their own run_id, since they are independent processes): `run_pipeline.py`, `import_csv.py`, `dedup_releases.py`, `verify_cache.py`, `filter_csv.py`, `resolve_collisions.py`, `tsv_to_sqlite.py`, `check_cache_drift.py`, `catalog_parity_diff.py`, `build_library_db.py`, `vendor_parity_residue.py`, `dedup_artist_children.py`. The shim itself lives in `lib/observability.py`.
