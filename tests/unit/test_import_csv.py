...
class TestMainArgParsing:
    ...
    def test_artists_only_mode_dispatches_to_import_artist_details(self, tmp_path) -> None:
        """``--artists-only`` runs *only* ``import_artist_details``: it stubs
        artist rows from ``release_artist``, UPDATEs profile from
        ``artist.csv``, and loads the artist child CSVs (alias, NV, member).

        This mode is the direct-pg-path counterpart to the artist-details
        block that ``--base-only`` runs inline. In CSV mode the artist
        tables are loaded as part of ``--base-only``; in direct-pg mode the
        converter has already loaded release tables and this is the only
        step needed to populate the artist side.

        See LML#497 for the rebuild-cache.sh + run_pipeline.py wiring that
        invokes this mode.
        """
        from unittest.mock import MagicMock, patch
        ...
        with (
            patch(
                "sys.argv",
                ["import_csv.py", "--artists-only", str(csv_dir), "postgresql:///test"],
            ),
            patch.object(_ic.psycopg, "connect", return_value=mock_conn),
            patch.object(_ic, "import_artist_details", return_value=42) as mock_artist,
            patch.object(_ic, "_import_tables") as mock_import,
            patch.object(_ic, "_import_tables_parallel") as mock_parallel,
            patch.object(_ic, "import_artwork") as mock_artwork,
            patch.object(_ic, "import_masters") as mock_masters,
            patch.object(_ic, "populate_cache_metadata") as mock_metadata,
        ):
            _ic.main()

        mock_artist.assert_called_once()
        artist_call = mock_artist.call_args
        assert artist_call[0][0] is mock_conn
        assert artist_call[0][1] == csv_dir
        # Other import paths must not be invoked in artists-only mode.
        mock_import.assert_not_called()
        mock_parallel.assert_not_called()
        mock_artwork.assert_not_called()
        mock_masters.assert_not_called()
        mock_metadata.assert_not_called()

    def test_artists_only_mutually_exclusive_with_base_only(self) -> None:
        """``--artists-only`` and ``--base-only`` (and ``--tracks-only``) are
        mutually exclusive — they live in the same mode group so the CLI
