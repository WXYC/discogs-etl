...
- **[`docs/migrations.md`](docs/migrations.md)** — Alembic layout + per-revision history (0001 baseline through 0012 `entity.release_identity`), dual-write convention, one-shot stamp procedure, defensive guards in `0001_initial.py` (`is_offline_mode()` + schema-presence short-circuit), shared `lib/alembic_helpers.py` (`refuse_offline` / `resolve_db_url`) used by 0010 / 0011 / 0012, deploy wiring via `rebuild-cache.yml`
