...
- **[`docs/migrations.md`](docs/migrations.md)** — Alembic layout + per-revision history (0001 baseline through 0008 `artwork_checked_at`), dual-write convention, one-shot stamp procedure, defensive guards in `0001_initial.py` (`is_offline_mode()` + schema-presence short-circuit), deploy wiring via `rebuild-cache.yml`
