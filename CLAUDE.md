...
- **New read-only exception (discogs-etl#327):** `scripts/run_pipeline.py::write_keep_release_ids` performs a read-only `SELECT` against `lml_cache.library_release_override` to build a never-evict allowlist for the dedup/prune seams (see `docs/architecture.md` "Library Release Overrides"). It queries the table directly and degrades to an empty allowlist only when the table/schema is genuinely absent (`UndefinedTable`/`InvalidSchemaName`); any other error (column typo, missing grant) propagates and aborts the rebuild rather than silently evicting every pin. This is the first — and only — place this repo reads `lml_cache.*`. It does not migrate, create, write to, or truncate that table, so the "never migrate/truncate/back up/audit `lml_cache.*`" rule above is unaffected and still holds for every other table in that schema.
