...
- `scripts/enrich_library_artists.py` -- Enrich artist list with WXYC cross-references via CatalogSource
...
- `lib/catalog_source.py` -- CatalogSource protocol with TubafrenzySource (MySQL) and BackendServiceSource (PostgreSQL) implementations for querying the WXYC library catalog
...
- `scripts/export_to_sqlite.py` -- Generate `library.db` from WXYC catalog via SSH (default) or `--catalog-source`
- `scripts/extract_library_labels.py` -- Extract label preferences from flowsheet plays via CatalogSource
...
`library.db` is a SQLite export of the WXYC library catalog, generated by `scripts/export_to_sqlite.py`. The default source is tubafrenzy's MySQL database on Kattare (via SSH), but Backend-Service's PostgreSQL database can be used instead via `--catalog-source backend-service --catalog-db-url <url>`. The `--catalog-source` flag is supported by `export_to_sqlite.py`, `enrich_library_artists.py`, `extract_library_labels.py`, and `run_pipeline.py`. The legacy `--wxyc-db-url` flag is an alias for `--catalog-source tubafrenzy --catalog-db-url <url>`. Backend-Service as a catalog source requires migration 0031 (`artist_crossreference` table) from Backend-Service PR #265.
